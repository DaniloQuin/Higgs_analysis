...
tree = 'diHiggs'
hists = ["dijets_mass", "dijets_DeltaR", "dijets_Pt", "nb_jets", "diphotons_mass", "diphotons_DeltaR", "diphotons_Pt", "diphotons_pTt", "leading_photon_Et", "subleading_photon_Et", "leading_jet_Et", "subleading_jet_Et", "nb_electrons", "nb_muons", "photonpt1frac", "photonpt2frac", "diphoton_diBjets_mass", "diphoton_diBjets_Pt", "BDT_response"]

samples = {
    'sgn': [basepath + 'diHiggs_sgn_Hbbyy_final.root'],
    'bkg': [basepath + 'diHiggs_bkg_bbyy_final.root'],
    'bkg_ybbj': [basepath + 'diHiggs_bkg_bbyj_final.root'],
    'bkg_yycc': [basepath + 'diHiggs_bkg_ccyy_final.root'],
    'bkg_yyjj': [basepath + 'diHiggs_bkg_jjyy_final.root'],
}

outfile = ROOT.TFile('/home/saolivap/WorkArea/GitLab/diHiggsbbyyAnalysis/TreeAnalysis/util/HistCompare/plotsFinal/hists.root','RECREATE')
...
for sample in samples.keys():
    ...
    for hist in hists:
        ...
        elif hist == 'BDT_response':
            h[sample][hist] = ROOT.TH1F(n, n + "; %s_%s [GeV]" % (hist, sample), 100, -0.6, 0.4)            
        else:
    ...
    for sample in samples.keys():
        ...
        for fname in l:
            ...
            if t.Draw('%s>>%s' % (hist, n),
                      '', # was: weight_perEvent*weight_finalState  
                      'goff'
            ):
...
for hist in hists:
    ...
    if hist == 'dijets_DeltaR' or hist == 'diphotons_DeltaR' or hist == 'photonpt1frac' or hist == 'photonpt2frac':
        ...
        hs.GetXaxis().SetRangeUser(-0.6,0.4)
    else:
        hs.GetXaxis().SetRangeUser(0,700)
    ...
    texATLAS.Draw()
    c.Update()
    c.SaveAs('/home/saolivap/WorkArea/GitLab/diHiggsbbyyAnalysis/TreeAnalysis/util/HistCompare/plotsFinal/%s.png' % (hist))
